    ...
    <nav className="fixed top-0 inset-x-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-lg">
      <div className="h-16 flex items-center justify-between px-6 max-w-7xl mx-auto">
        ...
        <a href="/" className="flex items-center gap-3 select-none group">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-lg transition-transform duration-200 group-hover:scale-105">
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          >
            <div
              className={`
                w-3 h-3 rounded-full border-2 animate-ping-slow
        ...
        <div className="flex items-center gap-3 lg:hidden">
          <button
            ...
            className="inline-flex items-center justify-center w-10 h-10 rounded-lg text-slate-600 hover:text-emerald-600 hover:bg-slate-100 transition-colors"
            onClick={() => setOpen(v => !v)}
          >
            {open ? (
      ...
      <div
        id="mobile-menu"
        ref={panelRef}
        className={`lg:hidden absolute left-0 right-0 top-16 transition-all duration-300 overflow-hidden ${open ? "opacity-100 pointer-events-auto max-h-96" : "opacity-0 pointer-events-none max-h-0"}`}
        style={{ zIndex: 49 }}
      >
        <div className="bg-white/95 backdrop-blur-md border-b border-x border-slate-200 shadow-lg rounded-b-xl mx-6">
